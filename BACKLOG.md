...
- `[x]` **Feb 2026 AWS change — the corpus half is done; the upstream half is now vector-backed and
  waiting on the Checkov queue.** Closed 2026-09-02, github-aws 0.7.0, six vectors.
  Read AWS's condition-keys page (GitHub tab) verbatim rather than trusting the note above. It
  documents exactly ten GitHub keys — `actor`, `actor_id`, `job_workflow_ref`, `repository`,
  `repository_id`, `repository_owner_id`, `workflow`, `ref`, `environment`, `enterprise_id` — every
  one marked **Available in session: No**, so the reach exists only in the role trust policy and no
  downstream resource or session policy can re-check any of it. The corpus graded three of the ten.
  It now grades all ten plus `sub` and `aud`. What the reading turned up:
  - **AWS's own example pins a GitHub handle.** `"token.actions.githubusercontent.com:actor":
    "octocat"`, while the row above it says actor IDs "are generated by GitHub and are immutable" —
    a distinction only worth drawing because the handle is not.
    `gh-aws-docs-actor-name-pin-recycled` (dangerous) and its rename-proof pair
    `gh-aws-actor-id-pin-survives-rename`.
  - **Three keys carry no repository at all.** `ref` is the bare branch or tag, not the sub's
    `repo:OWNER/NAME:ref:...`, so an exact `refs/heads/main` pin admits main of every repository
    registered against the provider. `workflow` is the YAML `name:` field, chosen by whoever writes
    the file. `actor_id` is immutable and still repo-less. All wildcard-free, so a review grading by
    `*` in the pattern objects to nothing: `gh-aws-ref-key-alone-omits-the-repository`,
    `gh-aws-workflow-name-pin-attacker-chosen`.
  - **Immutability and scope are independent.** `repository_owner_id` is exactly the stable
    identifier AWS says it is, and is org-wide — the breadth of `gh-aws-org-wide-wildcard-repo`
    without the wildcard that makes it visible. `gh-aws-repository-owner-id-alone-is-org-wide`.
  - **AWS's six-key example evaluated as a block**, `gh-aws-all-docs-github-tab-example`: a genuinely
    strong conjunction, held at caution for what it teaches — StringLike on six wildcard-free values,
    and three of the six are names with immutable alternatives in the same table.
  New vocabulary tag `select-claim-key`.
  **The docs gap, stated carefully because it is softer than the note above claimed.** GitHub's AWS
  how-to says verbatim "Support for custom claims for OIDC is unavailable in AWS". That most
  plausibly means subject customization, not these keys, so it is not the flat contradiction the
  sweep note implied — but a reader who takes it at face value pins `sub` alone and never reaches
  for `repository_id` or `actor_id`. Recorded inside
  `gh-aws-all-docs-github-tab-example` with that hedge intact rather than as a contradiction vector.
  **Still open, and deliberately not acted on: the Checkov feeder.** `gh_sub_condition` in
  `checkov/common/util/oidc_utils.py` matches only the `...:sub` condition key (verified against
  3.3.16 / `d8aec9db`, 2026-08-31), so none of these ten keys are graded by CKV_AWS_358 or
